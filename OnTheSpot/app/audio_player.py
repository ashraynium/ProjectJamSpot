...
class MidiAudioPlayer:
    ...
    def _send_programs(self) -> None:
        ...
        for channel, program in self.programs_by_channel.items():
            if channel != 9:
                self.output.write_short(0xC0 + channel, int(clamp(program, 0, 127)))
    ...
    def seek(self, new_time: float) -> None:
        ...
        for note in self.notes:
            end_time = note.start_sec + note.duration_sec
            if note.start_sec <= new_time < end_time:
                self._note_on(note)
                self.pending_offs.append((end_time, note.pitch, note.channel))
    ...
    def update(self, current_time: float) -> None:
        ...
        while self.note_index < len(self.notes):
            ...
            self.pending_offs.append(
                (note.start_sec + note.duration_sec, note.pitch, note.channel)
            )
            self.pending_offs.sort(key=lambda item: item[0])
            self.note_index += 1
