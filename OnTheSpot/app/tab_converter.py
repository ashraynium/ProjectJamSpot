...
def _best_chord_positions(
    chord: List[NoteEvent], previous_fret: float
    ...
    used = set()
    for note in chord:
        choices = [position for position in possible_positions(note.pitch) if position[0] not in used]
        if choices:
            string_index, fret = min(
                choices, key=lambda position: abs(position[1] - previous_fret)
            )
